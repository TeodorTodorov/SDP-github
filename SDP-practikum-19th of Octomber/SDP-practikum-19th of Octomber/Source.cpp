...
#include<fstream>
#include<string>
#include"ConsoleIOUserInteraction.h"
...
void testSimpleTestFile() {
	...
	if (!is_txtFile.is_open()) { // of simply !is_txtFile
		std::cerr << "No such file found: " << filename;
		return;
	}

	// get char
	// get getline  - neformatiran vhod na potoka
	// >> predefiniran operator za ifstream - predlaga formatiran potok na dannite
	char ch;
	...
	// get word
	char wordBuffer[60];
	// words are separated by space determinirashtiqt simvol e ' ' 
...
int main()
{
	...
	ConsoleIOUserInteraction* ci = new ConsoleIOUserInteraction(filename, 4);
	ci->startApplication();
